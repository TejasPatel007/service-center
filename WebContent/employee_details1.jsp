...
{
	try{
		String cust_id = request.getParameter("cdetails");
		String empl_id = request.getParameter("edetails");
		String query_de = request.getParameter("qdetails");
		Connection con = null;
		String sql = null;
		String sql1 = null;
		Statement st = null;
		...
		if(cust_id==null || empl_id==null || query_de==null){
			throw new Exception();
		}
		...
		while(rs.next()){
     		...
     		<td>Customer Address : <%= cust_ad %></td></tr>
     		</form>
     		</table></br></br>
     		<%
     		
			sql1 = "SELECT * FROM customer_service where customer_id='"+cust_id+"' AND query_details='"+ query_de +"'";
        	st1= con.createStatement();
        	rs1 = st1.executeQuery(sql1);
        	while(rs1.next()){
	        	//Retrieve by column name
        		Timestamp se_re_da = rs1.getTimestamp("service_request_date");
        		...
        		//Display values%>
        		Query :  <%= query_de %></br>
        		Service Request Date :  <%= se_re_da %></br>
        		Employee Assign Date :  <%= empl_se_re_da %></br><%
        		if(se_fu_da==null){%>
	        		Service Fulfill date : </br></br><%
        		}else{%>
	       		 	Service Fulfill date :  <%= se_fu_da %></br></br><%
   			 	}
        	}    			 
		}
		sql = "SELECT * FROM employee where employee_id='"+ empl_id +"'";
		st = con.createStatement();
		rs = st.executeQuery(sql);
		while(rs.next()){
	     	//Retrieve by column name
     		...
     		<form method="post" name="form1">
     		Assigned Employee
     		Employee ID : <%= empl_id %></br>
     		Employee Name : <%= empl_na %></br>
     		Employee Contact Number : <%= empl_co_no %></br>
     		</form></br></br><%	
    	}
	...
	rs1 = st1.executeQuery(sql1);
	while(rs1.next()){
		String cust_lo_id = rs1.getString("customer_login_id");%>
    	<form method="post" action="customer_details1.jsp">
